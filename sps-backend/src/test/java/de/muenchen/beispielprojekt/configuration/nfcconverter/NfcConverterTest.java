...
class NfcConverterTest {
    ...
    void testFilterIfContenttypeInWhitelist() throws ServletException, IOException {
        ...
        Mockito.verify(chain, Mockito.times(1))
                .doFilter(reqCaptor.capture(), Mockito.any(ServletResponse.class));
        ...
        //
        assertArrayEquals(
                VALUE_NFD.getBytes(UTF8),
                IOUtils.toByteArray(reqCaptor.getValue().getPart(NAME_NFD).getInputStream()));
    }

    //
    // Test, das Request nicht konfigriertem ContentType auf unverändert bleibt,
    // d.h. nicht
    ...
    private List<Part> mockParts() throws IOException {
        ...
        Mockito.when(part.getInputStream())
                .thenReturn(new ByteArrayInputStream(VALUE_NFD.getBytes(UTF8)));
