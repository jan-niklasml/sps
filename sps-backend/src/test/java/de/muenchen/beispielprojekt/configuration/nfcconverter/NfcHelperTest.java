...
class NfcHelperTest {
    ...
    void nfcConverterStringBuffer() {
        ...
        assertEquals(
                FIRST_NFC.length(), NfcHelper.nfcConverter(new StringBuffer(FIRST_NFD)).length());

        assertEquals(SECOND_NFC, NfcHelper.nfcConverter(new StringBuffer(SECOND_NFD)).toString());
        assertEquals(
                SECOND_NFC.length(), NfcHelper.nfcConverter(new StringBuffer(SECOND_NFD)).length());

        assertEquals(THIRD_NFC, NfcHelper.nfcConverter(new StringBuffer(THIRD_NFD)).toString());
        assertEquals(
                THIRD_NFC.length(), NfcHelper.nfcConverter(new StringBuffer(THIRD_NFD)).length());
    }
    ...
    void nfcConverterCookieArray() {
        ...
        Arrays.asList(nfcCookies)
                .forEach(
                        nfcCookie -> {
                            assertEquals(NfcConverterTest.TOKEN, nfcCookie.getName());
                            assertEquals(
                                    Arrays.toString(NFC_OUTPUT_EXPECTED), nfcCookie.getValue());
                            assertEquals(THIRD_NFC, nfcCookie.getDomain());
                            assertEquals(THIRD_NFC, nfcCookie.getPath());
                        });
    }
